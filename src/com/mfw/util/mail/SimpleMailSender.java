...
public class SimpleMailSender extends BaseController {
     ...
     * author yangdw
     */
    public static Boolean simpleSendEmail(String toEMAIL,String TITLE,String CONTENT){
        String strEMAIL = Tools.readTxtFile(Const.EMAIL); //读取邮件配置
        //这个类主要是设置邮件
        MailSenderInfo mailInfo = new MailSenderInfo();
        ...
        if (null != strEMAIL && !"".equals(strEMAIL)) {
            String strEM[] = strEMAIL.split(",mfw,");
            if (strEM.length == 5) {
                mailInfo.setMailServerHost(strEM[0]);
                mailInfo.setMailServerPort(strEM[1]);
                mailInfo.setUserName(strEM[2]);
                mailInfo.setPassword(strEM[3]);
                mailInfo.setFromAddress(strEM[2]);
                enabled = strEM[4];
            }
        }
        ...
        mailInfo.setValidate(true);
        mailInfo.setToAddress(toEMAIL);
        mailInfo.setSubject("来自" + Tools.readTxtFile(Const.SYSNAME) + "系统的消息提醒：" + user.getNAME() + "给您发送了新的" + TITLE + "，请登陆系统查看。");
        mailInfo.setContent(CONTENT);
