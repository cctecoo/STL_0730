 ...
 * 菜单管理Service
 * @author  作者 于亚洲
 * @date 创建时间：2014年7月1日 下午17:47:39
 */
@Service("menuService")
public class MenuService {
	 ...
	 * 查询网页端的父级别的菜单
	 */
	public List<Menu> listAllWebParentMenu() throws Exception {
		return (List<Menu>) dao.findForList("MenuMapper.listAllWebParentMenu", null);
	 ...
	 * 只查询网页端的菜单
	 */
	public List<Menu> listAllWebMenu() throws Exception {
		List<Menu> rl = this.listAllWebParentMenu();
		for (Menu menu : rl) {
			List<Menu> subList = this.listSubMenuByParentId(menu.getMENU_ID());
			menu.setSubMenu(subList);
            for(Menu newMenu : subList){
