package com.mfw.controller.system.head;
...
import com.mfw.util.SmsUtil;
import com.mfw.util.Tools;
import com.mfw.util.mail.SimpleMailSender;

/**
 * 获取head信息，发送邮件、短信，系统设置
 * @author  作者 于亚洲
 * @date 创建时间：2014年8月16日 下午13:18:24
 */
@Controller
...
public class HeadController extends BaseController {
	...
	public Object sendEmail() {
		...
		if (null != strEMAIL && !"".equals(strEMAIL)) {
			String strEM[] = strEMAIL.split(",mfw,");
			if (strEM.length == 4) {
				if ("yes".endsWith(isAll)) {
					try {
						List<PageData> userList = new ArrayList<PageData>();
	...
	public ModelAndView goEditEmail() throws Exception {
		...
		if (null != strSMS1 && !"".equals(strSMS1)) {
			String strS1[] = strSMS1.split(",mfw,");
			if (strS1.length == 2) {
		...
		if (null != strSMS2 && !"".equals(strSMS2)) {
			String strS2[] = strSMS2.split(",mfw,");
			if (strS2.length == 2) {
	...
	public ModelAndView saveU(PrintWriter out) throws Exception {
		...
		Tools.writeFile(Const.EMAIL, pd.getString("SMTP") + ",mfw," + pd.getString("PORT") + ",mfw," + pd.getString("EMAIL") + ",mfw," + pd.getString("PAW")); //写入邮件服务器配置
		Tools.writeFile(Const.SMS1, pd.getString("SMSU1") + ",mfw," + pd.getString("SMSPAW1")); //写入短信1配置
		Tools.writeFile(Const.SMS2, pd.getString("SMSU2") + ",mfw," + pd.getString("SMSPAW2")); //写入短信2配置
		mv.addObject("msg", "OK");
