 ...
 * 岗位职责
 * @author  作者  蒋世平
 * @date 创建时间：2015年12月23日 下午17:45:29
 */
@Controller
	...
	*/
	...
	public ModelAndView readExcel(@RequestParam(value = "excel", required = false) MultipartFile file,HttpServletRequest request) throws Exception {
		...
		if (null != file && !file.isEmpty()) {
			...
			List<PageData> listDetail = (List) ObjectExcelRead.readExcel(filePath, fileName, 0, 0, 1); //执行读EXCEL操作,读出的数据导入List 1:从第2行开始；0:从第A列开始；1:第1个sheet
			...
			for (int i = 2; i < listPd.size(); i++) {
				...
				if(listPd.get(i).getString("var0")==""||listPd.get(i).getString("var0")==null){
					continue;
				}else if(listPd.get(i).getString("var1")==""||listPd.get(i).getString("var1")==null){
					emptyStr = "部门名称";
				}else if(listPd.get(i).getString("var2")==""||listPd.get(i).getString("var2")==null){
					emptyStr = "岗位编码";
				}else if(listPd.get(i).getString("var3")==""||listPd.get(i).getString("var3")==null){
					emptyStr = "岗位名称";
				}else if(listPd.get(i).getString("var4")==""||listPd.get(i).getString("var4")==null){
					emptyStr = "职责";
				}
				if(!emptyStr.isEmpty()){
					mv.addObject("msg", "岗位职责 第"+(i+1)+"行中[" + emptyStr + "]没有填写，请检查后再导入");
					mv.setViewName("save_result");
					return mv;
				}
				...
				if(flag!=null){
					...
					mv.addObject("msg", "第"+(i+1)+"行的岗位不存在，请先添加岗位");
					mv.setViewName("save_result");
					return mv;
				}			
			}
			int countPosdutyDetail = 0;
			for (int i = 2; i < listDetail.size(); i++) {
				//空白字段，返回导入页面
				String emptyStr = "";
				if(listDetail.get(i).getString("var0")==""||listDetail.get(i).getString("var0")==null){
					continue;
				}else if(listDetail.get(i).getString("var1")==""||listDetail.get(i).getString("var1")==null){
					emptyStr = "岗位职责";
				}else if(listDetail.get(i).getString("var2")==""||listDetail.get(i).getString("var2")==null){
					emptyStr = "序号";
				}else if(listDetail.get(i).getString("var3")==""||listDetail.get(i).getString("var3")==null){
					emptyStr = "职责明细";
				}
				if(!emptyStr.isEmpty()){
					mv.addObject("msg", "职责详情 第"+(i+1)+"行中[" + emptyStr + "]没有填写，请检查后再导入");
					mv.setViewName("save_result");
					return mv;
				}
				...
				if(res !=null){
					...
					mv.addObject("msg", "第"+(i+1)+"行的职责详情对应的岗位职责不存在，请检查后再导入");
					mv.setViewName("save_result");
					return mv;
				}		
			}
			mv.addObject("msg", "成功导入" + countPosduty + "条岗位职责，" + countPosdutyDetail + "条职责明细");
		}
	 ...
	 * 导出到模板
	 */
	@RequestMapping("exportPositionDuty")
	public ModelAndView exportPositionDuty(){
		logBefore(logger, "exportPositionDuty start");
		try {
			String deptId = this.getPageData().getString("deptId");
			Map<String, Object> dataMap = positionDutyService.exportPositionDuty(Integer.valueOf(deptId));
			//执行excel操作
			return new ModelAndView(new ObjectExcelView(), dataMap);
		} catch (Exception e) {
			logger.error("exportPositionDuty error", e);
	...
	@ResponseBody
	@RequestMapping("copyResponsibilityDetail")
	public String copyResponsibilityDetail(){
		try {
			PageData pd = this.getPageData();
			positionDutyService.copyResponsibilityDetail(pd);
		} catch (Exception e) {
			logger.error("复制岗位职责明细出错", e);
			return "error";
		}
		return "";
	}
