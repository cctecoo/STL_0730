(function(snaker){
	...
	designer.config={
		...
		path:{
			attr:{
				path:{path:"M10 10L100 100",stroke:"#808080",fill:"none","stroke-width":3},
				arrow:{path:"M10 10L10 10",stroke:"#808080",fill:"#808080","stroke-width":3,radius:4},
				fromDot:{width:5,height:5,stroke:"#fff",fill:"#000",cursor:"move","stroke-width":3},
				toDot:{width:5,height:5,stroke:"#fff",fill:"#000",cursor:"move","stroke-width":3},
				bigDot:{width:5,height:5,stroke:"#fff",fill:"#000",cursor:"move","stroke-width":3},
				smallDot:{width:5,height:5,stroke:"#fff",fill:"#000",cursor:"move","stroke-width":4}
			},
			...
			props:{
				...
				name:{name:"name",label:"名称",tabs:{id:'base-prop',name:'基本属性'},value:"",
					editor:function(){return new designer.editors.inputReadonly()}},
				displayName:{name:"displayName",label:"显示",tabs:{id:'base-prop',name:'属性'},value:"",
					editor:function(){return new designer.editors.textEditor()}},
                type:{name:"type",label:"类型",tabs:{id:'base-prop',name:'属性'},value:"",
                	editor:function(){return new designer.editors.selectEditor("flowDesign/dictItem.do?busiValue=FLOW_LINE_TYPE");}},
                isCheckForm:{name:"isCheckForm",label:"验证表单",tabs:{id:'base-prop',name:'属性'},value:"", 
                	editor:function(){return new designer.editors.selectEditor("flowDesign/dictItem.do?busiValue=YES_OR_NO",utils.YES_OR_NO.YES);}},
                sortNum:{name:"sortNum",label:"显示序号",tabs:{id:'base-prop',name:'属性'},value:"", 
                	editor:function(){return new designer.editors.inputEditor()}}
				// expr:{name:"expr",label:"表达式",tabs:{id:'base-prop',name:'基本属性'},value:"",
                	//editor:function(){return new designer.editors.inputEditor()}}
			}
		},
		tools:{
		...
		props:{
			attr:{position: "absolute",top:85,right:30},
			props:{}
		},
		restore:"",
		activeRects:{
	...
	designer.rect=function(o, r,id){
        var _this = this, _o = snaker.extend(true, {},
        ...
        if(typeof(id) !== 'undefined' && null != id && id != '') {
        	_id = id;
        	var n = parseInt(id.replace(/[^0-9]/ig, ""));
        	designer.util.nextId(n);
        } else {
        	_id = 'rect' + designer.util.nextId(0);
        }
...
/*                var returnValue = window.showModalDialog(designer.config.ctxPath + '/config/form?lookup=1',window,'dialogWidth:1000px;dialogHeight:600px');
        ...
        this.toBeforeXml=function(){
            ...
            for(var k in _o.props){
				...
				case 'displayName':
					isCheck = true;
					msg = _o.type+" 显示名称 不能为空";
					break;
				case 'assignee':
				case 'assigneeDisplay':
					//	isCheck = true;
					//msg = "参与者不能为空";
					break;
				case 'form':
            	...
            	if(isCheck && utils.isEmpty(_o.props[k].value)) {
            		//alert(_o.props[k].value);
    ...
    designer.path=function(o, r, from, to,id){
        ...
        if(typeof(id) !== 'undefined' && null != id && id != '') {
        	...
        	_id = 'path' + designer.util.nextId(0);
        }
        ...
        this.toXml=function(){
            ...
            for(var o in _o.props){
                ...
                if(o=='displayName' && _o.props[o].value=='') {
                	var vv = _text.attr("text").replace(/>/g,"#5");
                    vv = vv.replace(/</g,"#6");
                    vv = vv.replace(/&/g,"#7");
                    r+=o+"=\""+vv+"\" ";
                	continue;
	...
	designer.props=function(o, r){
		...
		var _this = this, _pdiv = snaker("#properties").hide().bind("click",function(){return true}),  
		_tb = _pdiv.find(".properties_panel"), _r = r, _src;
		var showpropsHandler = function(e, props, src){
			...
			_tb.empty();
			_pdiv.show();
			$("#flow-prop-tabs").unbind("click");
			for(var l in props){
				if(!props[l].name) continue;
				if(props[l].value === undefined) {
					alert(l);
				}
				...
				var tabsId = "des-flow-"+props[l].tabs.id;
				var $tab = _tb.find("#"+tabsId);
				if(utils.isEmpty($tab.attr("id"))) {
					var panelTabsWrap = _tb.find(".panel-tabs-wrap");					
					if(utils.isEmpty(panelTabsWrap.attr("class"))) {
						//alert("append");
						_tb.append("<div class=\"panel-tabs-wrap\"><div class=\"panel-tabs-tab\"><ul id='flow-prop-tabs' class=\"nav nav-sm nav-tabs\" role=\"tablist\">"+
						"<li class=\""+(props[l].tabs.id=='base-prop'?'active':'')+"\"><a href=\"#"+tabsId+"\" role=\"presentation\" data-toggle=\"tab\"> "+props[l].tabs.name+"</a></li></ul>"+	
						"</div><div class=\"tab-content panel-tab-content bg-color-white p-2\"><div role=\"tabpanel\" class=\"tab-pane "+(props[l].tabs.id=='base-prop'?'active':'')+"\" id=\""+tabsId+"\">"+
						"<div class=\"properties_all\" id='prop-panel-"+tabsId+"'></table>"+
						"</div></div>"+
						"</div></div>");
					} else if(utils.isEmpty(_tb.find("#"+tabsId).attr("id"))) {
						var $navTabs = panelTabsWrap.find(".nav-tabs");
						$navTabs.append("<li><a href=\"#"+tabsId+"\" role=\"presentation\" data-toggle=\"tab\"> "+props[l].tabs.name+"</a></li>");
						var $tabContent = panelTabsWrap.find(".tab-content");
						$tabContent.append("<div role=\"tabpanel\" class=\"tab-pane\" id=\""+tabsId+"\"><div class=\"properties_all\" id='prop-panel-"+tabsId+"'></table></div>");
					}
				}
				var _tbp = _tb.find("#prop-panel-"+tabsId);
				var glyphicon = props[l].glyphicon;
				var isCustom = props[l].isCustom;
				isCustom = utils.isEmpty(isCustom)?false:isCustom;
				if(isCustom) {
					_tbp.append("<div id='p"+l+"' class='editor'></div>");
				} else {
					_tbp.append("<div class='form-group "+(!utils.isEmpty(glyphicon)?'has-feedback':'')+"'><label class='col-sm-3  control-label'>"+props[l].label+"</label><div id='p"+l+"' class='editor col-sm-9'></div></div>");
				}
				if(props[l].editor){
					props[l].editor().init(props,l,"p"+l,src,_r)
				}
				$('#flow-prop-tabs a').click(function (e) {
					  e.preventDefault();
					  $(this).tab('show');
				});
			}
	...
	designer.editors={
		textEditor:function(){
			...
			this.init=function(props, _k, _div, _src, _r){
				...
				var require = (typeof(_props[_k].isRequire)==='undefined')?'':(_props[_k].isRequire?'require':'');
				var formCheck = (typeof(_props[_k].formCheck)==='undefined')?'':_props[_k].formCheck;
				if(utils.isEmpty(props[_k].value)) {
					props[_k].value = _src.text();
				}
				snaker('<input class="form-control input-sm '+require+'" '+formCheck+' />').val(_src.text()).change(function(){
	...
	designer.init=function(c, o){
		...
		snaker('#snakerflow').click(function(event){
		//snaker(document).click(function(event){
		...
		if(designer.config.editable){
			...
			snaker(c).droppable({
				...
				drop:function(c,i){
					...
					offsetLeft = i.helper.offset().left-offsetLeft;
					offsetTop = i.helper.offset().top-offsetTop;
					snaker(_r).trigger("addrect",[i.helper.attr("type"),{attr:{x:offsetLeft,y:offsetTop}}])
				}
			});
			snaker("#save,#update,#export").click(function(){
				var i="<process ";
				var isCheck = false;
				var msg = '';
				for(var c in designer.config.props.props){
					...
					//case 'orgDisplay':
						isCheck = true;
						//alert(designer.config.props.props[c].value);
						msg = '组织机构不能为空!';
						break;
					case 'formId':
						isCheck = true;
					...
					if(isCheck && utils.isEmpty(designer.config.props.props[c].value)) {
						BootstrapDialogUtil.warningAlert(msg);
						return;
					}
				...
				for(var node in _states){
					if(_states[node]){
						var xml = _states[node].toBeforeXml();
						...
						for(var transition in _paths){
							if(_paths[transition]){
								...
								if(from==_states[node].getId()){
									...
									if(transitionXml==""){
										BootstrapDialogUtil.warningAlert("连接线名称不能为空");
										return
									}else{
										i+="\n";
										i+=transitionXml;
				...
				designer.config.tools.save.onclick(i,snaker(this).attr("id"));
			});
		...
		if(o.restore){
			...
			if(data.states){
				for(var s in data.states) {
					var rect = new designer.rect(snaker.extend(true,{},designer.config.tools.states[data.states[s].type],data.states[s]),_r,s);
			...
			if(data.paths){
				for(var s in data.paths){
					var p = new designer.path(snaker.extend(true,{},designer.config.tools.path,data.paths[s]),_r,rmap[data.paths[s].from],rmap[data.paths[s].to],s);
		...
		//TODO 默认显示流程属性
		setTimeout(function() {
			snaker('#snakerflow').trigger('click');
		}, 200);
	};
