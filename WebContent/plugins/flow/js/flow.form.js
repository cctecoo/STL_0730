				...
				for(var i=0;i<fieldNames.length;i++) {
					$this.find("input[name='"+fieldNames[i]+"'],select[name='"+fieldNames[i]+"'],textarea[name='"+fieldNames[i]+"'],#"+fieldNames[i]+",span[data-name='"+fieldNames[i]+"']").each(function(){
						...
						if(tagName == 'span') {
							...
							} else if($findElement.hasClass("cnoj-deptId-defvalue")) {
								$findElement.val(setting.deptId);
							} else if($findElement.hasClass("cnoj-childCompany-defvalue")) {
								$findElement.val(setting.childCompany);
							}
						}
					});
				}
...
function submitTask(formObj,url,param) {
	...
	refreshUrl = utils.isEmpty(refreshUrl)?"todo.do":refreshUrl;
		...
		$("#handle-form-iframe").load(function(){
	    	...
	    	if(utils.isNotEmpty(result)) {
	    		...
	    		if(output.result=='1') {
					...
					window.location.href = 'processList.do';
				}
	    	} else {
...
function createNextAssignerTree(tagObj,processId,orderId,selectStyle,isShow) {
	...
	if(taskKey != END_NODE_KEY) {
		...
		var url = "selectNextAssigner.do?"+params;
		var checkOpt = null;
...
function isHandleNextAssigner($nextLineRow) {
	...
	var processId = $("#process-id").val();
	$.ajax({
		url:'isSelectAssigner.do?nextTaskKeys='+selectedNextTask+"&processId="+processId,
		type:'get',
		async:false,
