...
<html lang="zh-CN">
	...
	<body>
		...
		<div id="page-content">
			<div class="row-fluid">
				<div class="main-container-left" style="height:600px; overflow-y:scroll;">
					<div class="m-c-l-top">
		...
		<script type="text/javascript">
				...
				$("#task_grid").bootgrid({
					...
					formatters:{
						links: function(column, row)
				        {
				            ...
				            return '<a title="' + row.FILENAME + '" style="cursor:pointer;" onclick="window.location.href=\''+url +'\'">' + row.FILENAME + '</a>';
				        },
						btns: function(column, row){
							var uploadPerson = row.UPLOADPERSON;
							var user = "${USER_NAME}";
							if(user==uploadPerson)
								//if(row.upload=='noissued'){
									return '<a style="cursor:pointer;" title="删除" onclick="del(\'' + row.ID +'\');" class="btn btn-mini btn-danger" data-rel="tooltip" data-placement="left">'+
		                                   '<i class="icon-trash"></i></a>'+
											'<a style="cursor:pointer;" title="修改" onclick="edit(\'' + row.ID +'\');" class="btn btn-mini btn-info" data-rel="tooltip" data-placement="left">'+
											'<i class="icon-edit"></i></a>';
								//}else
									//return '<a style="cursor:pointer;" title="修改" onclick="edit(\'' + row.ID +'\');" class="btn btn-mini btn-info" data-rel="tooltip" data-placement="left">'+
									//	'<i class="icon-edit"></i></a>';
						},
		    ...
		    function edit(id){
		    	 ...
		    	 var diag = new top.Dialog();
						 diag.Drag=true;
						 diag.Title ="编辑";
						 diag.URL = '<%=basePath%>repository/goEditInformation.do?ID='+id;
						 diag.Width = 380;
						 diag.Height = 310;
			...
			function upload(){
				 ...
				 diag.Drag=true;
				 diag.Title ="文件上传";
				 diag.URL = 'repository/showUploadInformation.do?DOC_TYPE=0&DEPT_CLASSIFY='+deptId;
				 diag.Width = 500;
				 diag.Height = 400;
				 diag.CancelEvent = function(){ //关闭事件
