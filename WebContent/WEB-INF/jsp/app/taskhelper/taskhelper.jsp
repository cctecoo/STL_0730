...
<html lang="zh_CN">
	<head>
		...
		<style>
			...
			.keytask table tr td{line-height:1.3}
			
			.web_menu a{width: 49.5%;} 
	...
	<body>
		<form id="helperForm">
			<div class="web_menu">
		   		<a style="width:49%; padding:6px 0; border-right: 1px solid #12e6d2" onclick="searchData('collection');" class="<c:if test="${type!='all'}">active</c:if>">常用日清助手</a>
		   		<a style="width:50%; padding:6px 0; border-right: 1px solid #12e6d2" onclick="searchData('all');" class="<c:if test="${type=='all'}">active</c:if>">日清助手</a>
						...
						<td colspan="2" style="padding: 6px 0;">
							<div style="float:left;margin-left: 50px;">
								<c:choose>
									...
									<c:otherwise>
										<input type="button" class="btn	" onclick="removeCollection(${duty.ID})" value="移除收藏" />
									</c:otherwise>
								</c:choose>
							</div>
							<div class="startDIV">
                               <input class="btn" id="startBtn_${duty.ID}" type="button" onclick="startWork('${duty.ID}')" value="启动工作">
                               <input type="button" class="btn btn-primary" id="pauseBtn_${duty.ID}" onclick="pauseWork('${duty.ID}')" style="display: none;" value="暂停">
                               <input type="button" class="btn btn-info" id="continueBtn_${duty.ID}" onclick="continueWork('${duty.ID}')" style="display: none;" value="继续">
                               <input type="button" class="btn btn-danger" id="endBtn_${duty.ID}" onclick="endWork('${duty.ID}')" style="display: none;" value="结束">
		...
		<script type="text/javascript">
			...
			function addCollection(id){
				$.post(
					...
					{"id":id},
					function(data){
						if(data != 1 && data.indexOf('<!DOCTYPE html>')>0){//验证是否请求正常
							location.href = '<%=basePath%>app_login/login_index.do';
							return;
						}
						if(data == 1){//添加收藏成功
