...
<body>
	...
	<input type="hidden" name="empDeptArea" value="BUD" /><!-- 标识表单类型，用于获取编码 -->
							...
							<c:forEach begin="0" end="${costItemCount-1 }" varStatus="vs">
								<c:if test="${vs.index<6 }">
									<tr class="rowItem_${vs.index+1 }">
										<td style="width:35px; padding:0" rowspan="7" >${vs.index+1}</td>
									...
									<tr class="rowItem_${vs.index+1 }">
										<td>申请金额</td>
										<td>${budgetAdjustAmount_list[vs.index] }</td>
									</tr>
									<tr class="rowItem_${vs.index+1 }">
										<td>剩余预算</td>
										<td>${budgetSurplus_list[vs.index] }</td>
									</tr>
									<tr class="rowItem_${vs.index+1 }">
										<td>年初预算数</td>
										<td>${budget_list[vs.index] }</td>
									</tr>
									<tr class="rowItem_${vs.index+1 }">
										<td>调整后总数</td>
										<td>${budgetAfter_list[vs.index] }</td>
									</tr>
