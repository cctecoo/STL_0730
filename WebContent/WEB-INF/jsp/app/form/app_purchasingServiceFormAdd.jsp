					...
					<tbody>
						<c:if test="${costItemCount>0 }">
							...
							<tr class="rowItem_${vs.index+1 }">
								<td rowspan="8"style="width:35px; padding:0">${vs.index+1}</td>
								<td style="width:85px;">服务项目名称</td>
								<td>${caiGouZiXiangMing_list[vs.index]}</td>
							</tr>
							<tr class="rowItem_${vs.index+1 }">
								<td>计划周期</td>
								<td>${jiHuaKaiShi_list[vs.index] }至${jiHuaJieShu_list[vs.index] }</td>
							</tr>
							<tr class="rowItem_${vs.index+1 }">
								<td>具体服务事项</td>
								<td>${juTiFuWuShiXiang_list[vs.index] }</td>
							</tr>
							<tr class="rowItem_${vs.index+1 }">
								<td>预计金额</td>
