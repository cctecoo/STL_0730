...
<html lang="en">
	...
	<body>
		<div class="button-group" style="width: 100%;text-align: right;">
		    ...
		    <button class="button button-primary" type="button" onclick="clickSearch('YW_DSX', this);">待审核</button>
			<button class="button button-primary" type="button" onclick="clickSearch('YW_YSX', this);">已审核</button>
			<button class="button button-primary active" type="button" onclick="clickSearch('all', this);">全部</button>
			
			<span class="button-dropdown" data-buttons="dropdown" style="float: right;height: 40px;">
				<a style="background:url(static/img/app_moreoption1.png) no-repeat; display:block; width:30px; height:30px;margin:5px 5px 0 0;">
		...
		<div style="width: 98%;background:#f6f6f6;padding: 5px;">
		...
		<script type="text/javascript">
			...
			function apendElement(task){
				var appendStr = '<div class="keytask">' + 
					...
					'<span style="margin-left:3px;">' + task.DEPT_NAME + '</span></td></tr>' +
					'<tr><td>创建人:</td><td colspan="3"><span>' + task.CREATE_NAME + '</span>' +
					'<span>' + task.CREATE_TIME + '</span></td></tr>' +
			...
			function getCommand(row){
				...
				if(res == 'YW_DSX'){
					str += 	'<a style="cursor:pointer;" title="查看" onclick="view(\'' + row.ID +'\',\'' + row.TASK_CODE +'\');" class="btn btn-mini btn-info" data-rel="tooltip" data-placement="left">'+
		                '<i class="icon-eye-open"></i></a>'+
		                '<a style="cursor:pointer; margin-left:3px;" title="审核通过" onclick="save(\'' + row.ID +'\',\'YW_YSX\',\'' + row.TASK_CODE +'\',\'' + row.MAIN_EMP_ID +'\',\'' + row.MAIN_EMP_NAME +'\');" class="btn btn-mini btn-primary" data-rel="tooltip" data-placement="left">'+
		                '<i class="icon-check"></i></a>'+
		                '<a style="cursor:pointer; margin-left:3px;" title="审核退回" onclick="save(\'' + row.ID +'\',\'YW_YTH\',\'' + row.TASK_CODE +'\',\'' + row.MAIN_EMP_ID +'\',\'' + row.MAIN_EMP_NAME +'\');" class="btn btn-mini btn-danger" data-rel="tooltip" data-placement="left">'+
			...
			function save(id,status,TASK_CODE,MAIN_EMP_ID,MAIN_EMP_NAME){
				...
				if(status == 'YW_YTH'){
					$.ajax({
						url: '<%=basePath%>app_temp/checkTaskIsdelete.do?id='+id,
						type: 'post',
						success: function(data){
							if("Y"==data){
								alert('任务已删除！请刷新页面');
