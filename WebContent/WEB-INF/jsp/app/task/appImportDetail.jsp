	...
	<head>
		...
		*{margin:0;padding:0;list-style-type:none;}
		...
		function showTaskDialog(type){
			 /*
			 //员工日清-超期不能填写
			 if(type=='' && ${pd.overDays>30}){
				alert("超过任务结束时间30天，不能填报日清");
				 return;
			 }
			*/
			 //没有活动分解项，则不能进行日清
			 ...
			 //alert(taskCount);
	 		 window.location.href = '<%=basePath%>app_task/toAddImportTask.do?&show=${pd.show}'
 				 + '&showDept=${pd.showDept}&loadType=${pd.loadType}'
 				 + '&taskCount=' + taskCount
				 + "&projectEventId=${projectEvent.ID}&projectNodeId=${projectEvent.C_PROJECT_NODE_ID}"
				 + "&endDate=${projectEvent.END_DATE }&projectCode=${projectEvent.C_PROJECT_CODE}";
		}
		...
		//日清任务-审核
		function validate(taskEmpCode, taskTime, id, isPass){
			var cmd = "";
			if(isPass==1){
				cmd = "通过";
			}else{
			...
			if(confirm("确认"+cmd+"?")){
				...
				url += "&projectEventId=${projectEvent.ID}" + "&taskTime=" + taskTime + "&taskEmpCode=" + taskEmpCode +
...
<%-- 			<div class="back" style="top:5px">
			...
			<div class="web_menu" style="width:90%; margin-left:20px;">
				<ul id="myTab" class="nav nav-tabs">
					<li class="active" style="width:30%;"><a style="width:100%; padding-right:0; padding-left:0;" href="#detail" data-toggle="tab">任务详情</a></li>
		   ...
		   <div id="normal" class="normal" style="width: 100%;text-align: right">
				...
				<c:if test="${pd.show==3 }"><!-- 领导-日清看板 -->
					<c:if test="${projectEvent.actual_percent==100.00 && empty projectEvent.SCORE}">
						<a id="assessBtn"  onclick="assessEvent();" class="btn btn-mini btn-info" style="margin: 15px 2px 0 0; float:right ">保存评分</a>
					</c:if>
		...
		<div id="zhongxin" style="width:98%; margin: 30px auto; border: none" class="tab-content">
			...
			<div class="tab-pane fade in active" id="detail">
				...
				<tr>
					<td style="text-align: center; width: 25%;"><label>项目名称：</label></td>
					<td style="text-align: left;">${projectEvent.PROJECT_NAME }</td>
				</tr>
				<tr>
					<td style="text-align: center;"><label>节点名称：</label></td>
					<td style="text-align: left;">${projectEvent.NODE_TARGET }</td>
				</tr>
				<!-- 
				<tr>
				...
				<tr>
					...
					<td style="text-align: left;">${projectEvent.EVENT_NAME }</td>
				</tr>
				
				<tr>
					<td style="text-align: center;"><label>活动类型：</label></td>
					<td style="text-align: left;">${projectEvent.TYPE }</td>
				</tr>
				<!-- 
				<tr>
					<td style="text-align: center;"><label>活动成本：</label></td>
					<td style="text-align: left;">${projectEvent.COST }</td>
				</tr>
				 -->
				<tr>
					<td style="text-align: center;"><label>活动周期：</label></td>
					<td style="text-align: left;">${projectEvent.START_DATE } 至 ${projectEvent.END_DATE }</td>
				</tr>
				<!-- 
				<tr>
					<td style="text-align: center;"><label>当前SPI：</label></td>
					<td style="text-align: left;">${projectEvent.spi }</td>
				</tr>
				<tr>
					<td style="text-align: center;"><label>预计完成时间：</label></td>
					<td style="text-align: left;">${projectEvent.expect_date }</td>
				</tr>
				 -->
				<tr>
					...
					<td style="text-align: left;">
						<div class="progress progress-info progress-striped opacity" 
							...
							<div class="bar" style="width: ${projectEvent.plan_percent }%; line-height: 20px;">
							...
							<c:choose>
								<c:when test="${projectEvent.overProgress>=0 }">
									<div class="progress-bar success" role="progressbar" aria-valuenow="60" aria-valuemin="0" 
										aria-valuemax="100" style="width: ${projectEvent.actual_percent }%; line-height: 20px; text-align: center">
								</c:when>
								<c:otherwise>
				...
				<input type="hidden" id="score" value="${projectEvent.SCORE}" style="width:50px;">
								...
								<table style="width: 100%; border-top: 1px solid #ccc;">
									...
									<tr>
										...
										<td colspan="3">
											...
											<span>，${empTask.CREATE_TIME }，提交人:<c:if test="${projectEvent.EMP_NAME!=empTask.CREATE_USER }">PI补录-</c:if>${empTask.CREATE_USER }</span>
										</td>
									</tr>
									<c:if test="${not empty empTask.TASK_DESC}">
										<tr>
											<td>工作结果：</td>
											<td colspan="3"><span>${empTask.TASK_DESC }</span></td>
										</tr>
									</c:if>
									...
									<tr>
										<td>附件</td>
										<td colspan="3">
											<c:if test="${not empty empTask.fileList }">
												<c:forEach items="${empTask.fileList }" var="file">
													<a style="cursor:pointer; color:#08c;" onclick="loadFile('${file.FILENAME_SERVER }')">${file.FILENAME }</a>
			                                		<br>
			                                	</c:forEach>
											</c:if>
										</td>
									...
									<tr>
										...
										<td colspan="2" style="text-align: right">
											...
											<c:if test="${pd.show==3 && empTask.STATUS=='草稿' }">
												<a style="cursor:pointer;" onclick="validate('${projectEvent.EMP_CODE }','${empTask.CREATE_TIME }', ${empTask.ID}, 1)" 
													title="审核通过" class='btn btn-mini btn-success' data-rel="tooltip" data-placement="left">
													通过<i class="icon-check"></i>
												</a>
												<a style="cursor:pointer;" onclick="validate('${projectEvent.EMP_CODE }','${empTask.CREATE_TIME }', ${empTask.ID}, 0)" 
