		...
		<form action="projectType/${msg}.do" name="completeForm" id="completeForm" method="post">
			...
			<table style="margin:20px auto;">
				...
				<tr>
					...
					<td><select class="select" name="PROJECT_NODE_LEVEL_FRAME_ID" id="PROJECT_NODE_LEVEL_FRAME_ID" value="${pd.PROJECT_NODE_LEVEL_FRAME_ID }" data-placeholder="请选择结构" style="vertical-align:top;z-index:999">
						<c:forEach items="${frameList}" var="frame">
							<option value="${frame.ID }" <c:if test="${pd.PROJECT_NODE_LEVEL_FRAME_ID==frame.ID }">selected</c:if>>${frame.LEVEL_FRAME }</option>
						</c:forEach>
					</select></td>
				</tr>
				<tr>
					<td><span style="color: red;">*</span>是否需要审批：</td>
					<td><input type="radio" name="ISCHECK" id="ISCHECK" value="1"<c:if test="${pd.ISCHECK == 1 || msg == 'add'}">checked="checked"</c:if>>是
						<input type="radio" name="ISCHECK" id="ISCHECK" value="0"<c:if test="${pd.ISCHECK == 0}">checked="checked"</c:if>>否
				</tr>
				<tr id="checkPersonTr">
					<td><span style="color: red;">*</span>审批人：</td>
					<td><input type="text" name="EMP_NAME" id="EMP_NAME" value="${pd.EMP_NAME }" maxlength="32" onclick="deptAndEmp()"/></td>
				</tr>
				<tr>
