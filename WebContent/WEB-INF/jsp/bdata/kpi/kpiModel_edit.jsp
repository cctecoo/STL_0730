	...
	<form action="kpiModel/${msg}.do" name="Form" id="Form" method="post">
		...
		<table style="width: 98%; margin:0 auto;">
			<tr>
				<td style="text-align: center; width:230px;">
					...
					<input type="radio" name="ENABLE" value="0" style="opacity: 1;position: relative;" 
						<c:if test="${pd.ENABLE == 0 || empty pd.ENABLE}">checked="checked"</c:if>/>否
				</td>
			...
			<tr>
				<td style="text-align: center;">
					非车间考核:
					<input type="radio" name="IS_WORK_SHOP_MODEL" value="0" style="opacity: 1;position: relative;"
						<c:if test="${pd.IS_WORK_SHOP_MODEL == 0}">checked="checked"</c:if> />员工考核
				</td>
				<td style="text-align: center;"><span style="margin-left:20px;">模板类型:</span>
					<input type="radio" name="MODEL_TYPE" value="EMP_MONTH" style="opacity: 1;position: relative;"
						<c:if test="${pd.MODEL_TYPE == 'EMP_MONTH'}">checked="checked"</c:if> />月度考核
					<input type="radio" name="MODEL_TYPE" value="EMP_YEAR" style="opacity: 1;position: relative;"
						<c:if test="${pd.MODEL_TYPE == 'EMP_YEAR'}">checked="checked"</c:if> />年度考核
				</td>
			</tr>
			<tr>
				<td style="text-align: center;">
					车间考核:
					<input type="radio" name="IS_WORK_SHOP_MODEL" value="1" style="opacity: 1;position: relative;"
						<c:if test="${pd.IS_WORK_SHOP_MODEL == 1}">checked="checked"</c:if> />车间员工考核
					<input type="radio" name="IS_WORK_SHOP_MODEL" value="2" style="opacity: 1;position: relative;"
						<c:if test="${pd.IS_WORK_SHOP_MODEL == 2}">checked="checked"</c:if> />车间考核
				</td>
				<td style="text-align: center;">
					<span style="margin-left:20px;">模板类型:</span>
					<input type="radio" name="MODEL_TYPE" value="QA" style="opacity: 1;position: relative;"
						<c:if test="${pd.MODEL_TYPE == 'QA'}">checked="checked"</c:if> />QA考核
					<input type="radio" name="MODEL_TYPE" value="EHS" style="opacity: 1;position: relative;"
						<c:if test="${pd.MODEL_TYPE == 'EHS'}">checked="checked"</c:if> />EHS考核
					<input type="radio" name="MODEL_TYPE" value="DVC" style="opacity: 1;position: relative;"
						<c:if test="${pd.MODEL_TYPE == 'DVC'}">checked="checked"</c:if> />设备考核
				</td>
			</tr>
