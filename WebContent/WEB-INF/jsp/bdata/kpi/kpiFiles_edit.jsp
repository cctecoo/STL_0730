...
<html lang="en">
	<head>
		...
		#zx td label{text-align:right; margin-right:10px;} 
		...
		<div id="zhongxin">
			<table style="margin: 0 auto; width: 98%; "><br>
				...
				<tr>
					...
					<td>
						<input type="text"  name="KPI_CATEGORY_NAME" id="kpi_category_name" value="${category.NAME}" maxlength="32" readonly/>
						<input type="hidden" name="KPI_CATEGORY_ID" id="kpi_category_id" value="${category.ID}" maxlength="32" />
					</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>
						<textarea type="text" name="KPI_SQL" id="kpi_sql" style="height: 120px;width: 400px;">${pd.KPI_SQL }</textarea>
					</td>
				</tr>
				<tr>
					...
					<td>
						<input type="radio" name="ENABLED" value="1" style="width:18px" 
							<c:if test="${pd.ENABLED == 1 || empty pd.ENABLED}">checked="checked"</c:if>>是
	...
	<script type="text/javascript">
		...
		function save(){
			...
			if($("#kpi_name").val() == ""){
				...
				$("#kpi_name").focus();
				return false;
			}else if($("#kpi_name").val().indexOf(',')!=-1){
				$("#kpi_name").tips({
					side:3,
		            msg:'不能包含英文逗号！',
		            bg:'#AE81FF',
