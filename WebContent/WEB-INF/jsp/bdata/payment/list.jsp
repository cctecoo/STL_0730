	...
	<div class="container-fluid" id="main-container">
		<div id="page-content" class="clearfix">
			...
			<table id="task_grid"
				...
				<thead>
					<tr>
						<th data-column-id="CODE" data-order="desc">编号</th>
						<th data-column-id="NAME">名称</th>
						<th data-column-id="ITEM_NAME">唯一标识</th>
						<th data-column-id="TYPE" data-formatter="type">类型</th>
						<th data-column-id="CREATE_USER"  data-formatter="userName">创建人</th>
	...
	<script type="text/javascript">
	        ...
	        $("#task_grid").bootgrid({
				...
				formatters:{
					btns: function(column, row){
						return '<a style="cursor:pointer;margin-left: 2px;" title="修改" onclick="edit(\'' + row.ID +'\');" '
							+ ' class="btn btn-mini btn-primary" data-rel="tooltip" data-placement="left">修改</a>'
		                    + '<a style="cursor:pointer;margin-left: 2px;" title="删除" onclick="del(\'' + row.ID +'\');" '
		                    + ' class="btn btn-mini btn-danger" data-rel="tooltip" data-placement="left">删除</a>';
					},
		...
		//检索
	   	function toSearch(){
			...
			$("#task_grid").bootgrid("search", {"keyword":keyWord});
	    }
		...
		function del(id){
			if(confirm("确定要删除?")){
				top.jzts();
				//检查运算符是否已经在公式中使用，如果已经使用不能删除
				$.ajax({
		            type: "POST",
		            url: '<%=basePath%>payment/checkHasUsed.do?ID=' + id,
		            cache: false,
		            async:false,
