...
<body>
	...
	<div class="main-container-left" style="width:250px">
<div class="m-c-l-top">
	<img src="static/images/ui1.png" style="margin-top:-5px;">岗位职责
					...
					<th><a class="btn btn-mini btn-danger" onclick="del();">删除</a></th>
					<!-- <th><a class="btn btn-mini btn-warning" onclick="copy();">复制</a></th> -->
		<!-- <a class="btn btn-mini btn-primary" onclick="fromExcel();" title="从EXCEL导入" style="margin-left:90px;">导入</a> -->
			</div>
	
		<table class="table table-striped table-bordered table-hover" data-min="11" data-max="30" cellpadding="0" cellspacing="0" >
		  <thead>
					...
					<c:forEach items="${varList}" var="position" varStatus="vs">
						<tr onclick="showDetail('${position.ID}',$(this));" style="cursor:pointer">
							<td class='center' style="width: 30px;">
								<label>
									<input type='checkbox' name='ids' kpiid ="${position.ID}"  value="${position.ID}" <c:if test="${pd.ID == position.ID}">checked="checked"</c:if>/><span class="lbl"></span>
								</label>
							</td>
	...
	<div class="main-content" style="margin-left:252px">
			...
			<div class="m-c-l_show"></div> <div id="subjectName"></div>
				<div style="position:absolute; top:0px; right:25px;">
					<span class="badge" style="background-color:#fee188; color: #963!important; margin-left:3px;">
						<i class="icon-info-sign">工作明细用于显示日清助手，在日常工作日清中显示</i>
					</span>
		...
		function showDetail(id,obj){
			...
			$.ajax({
				...
				success:function (data){     //回调函数，result，返回值
					var obj = eval('(' + data + ')');
					var shtml = '';
					$("#subjectName").html(obj.pd.responsibility);
					...
					$.each(obj.list, function(i, list){
						...
						shtml += '<tr class="record">'
					 		+ '<td class="center">' + Number(i+1) + '</td>' 
					 		+ '<td>' + list.detail + '</td><td>' + list.requirement + '</td>'
					 		+ '<td>' + list.standard_time + '</td>' + '<td>' + list.unit + '</td>'
					 		+ '<td>' + list.frequency + '</td><td>' + list.guide + '</td>'
					 		+ '<td>' + list.target + '</td><td>' + approveStr + '</td>'
					 		+ '<td style="display:none">' 
					 		+ '<input type="text" name="kID" id="kID" style="width:1px" value="' + list.ID + '" /></td>'
						 	+ '<td style="width: 70px">'
							+ '<a style="cursor:pointer" title="编辑" class="btn btn-mini btn-info" data-rel="tooltip" data-placement="left" '
							+ ' onclick="editDetail(' + list.ID + ')" ><i class="icon-edit"></i></a>'
                        	+ '<a style="cursor:pointer" title="删除" class="btn btn-mini btn-danger" data-rel="tooltip" data-placement="left" '
                        	+ ' onclick="del1(' + list.ID + ')" ><i class="icon-trash"></i></a>' 
                        	+ '</td></tr>';
					 });
