  ...
  <div class="row-fluid">
	...
	<div id="breadcrumbs">
		<table class="center" style="width:100%;">
			<tr height="35">
				<c:if test="${QX.add == 1 }">
				<td style="width:69px;"><a href="javascript:addRole();" class="btn btn-small btn-info">新增组</a></td>
				</c:if>
					<c:choose>
					<c:when test="${not empty roleList}">
					<c:forEach items="${roleList}" var="role" varStatus="vs">
						<td style="width:100px;" class="center" <c:choose><c:when test="${pd.ROLE_ID == role.ROLE_ID}">bgcolor="#98c0d9" onMouseOut="javascript:this.bgColor='#98c0d9';"</c:when><c:otherwise>bgcolor="#E5E5E5" onMouseOut="javascript:this.bgColor='#E5E5E5';"</c:otherwise></c:choose>  onMouseMove="javascript:this.bgColor='#98c0d9';" >
		...
		<table>
			...
			<a class="btn btn-mini btn-info" onclick="editRole('${pd.ROLE_ID }');">修改组名称<i class="icon-arrow-right  icon-on-right"></i></a>
				...
				<c:choose> 
					...
					<c:otherwise>
					 <c:if test="${QX.del == 1 }">
					 <a class='btn btn-mini btn-danger' title="删除" onclick="delRole('${pd.ROLE_ID }','z','${pd.ROLE_NAME }');"><i class='icon-trash'></i>删除组</a>
					 </c:if>
					</c:otherwise>
				</c:choose>
			</td>
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
					...
					<c:if test="${pd.ROLE_ID != var.ROLE_ID }">
						...
						<tr>
							...
							<td>
								...
								<c:if test="${QX.edit == 1 }">
									<a class="btn btn-mini btn-purple" onclick="editRights('${var.ROLE_ID }');" title="权限配置"><i class="icon-pencil"></i>权限配置</a>
									<a class='btn btn-mini btn-info' title="编辑名称" onclick="editRole('${var.ROLE_ID }');"><i class='icon-edit'></i>编辑</a>
								</c:if>
								<c:choose> 
									...
									<c:otherwise>
									 <c:if test="${QX.del == 1 }">
									 <a class='btn btn-mini btn-danger' title="删除" onclick="delRole('${var.ROLE_ID }','c','${var.ROLE_NAME }');"><i class='icon-trash'></i>删除</a>
									 </c:if>
									</c:otherwise>
								</c:choose>
							</td>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;"><a class="btn btn-small btn-info" onclick="addRole2('${pd.ROLE_ID }');">新增角色</a></td>
			</tr>
		</table>
		</c:if>
		</div>
