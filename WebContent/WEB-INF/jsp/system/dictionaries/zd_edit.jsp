	...
	function has(){
		...
		$.get(url,function(data){
			if(data=="error"){
			    ...
			    var bianMa = $("#BIANMA").val();
			    //设定值为空时，设置为编码
			    if($("#DISPLAY_VALUE").val()==''){
                    $("#DISPLAY_VALUE").val(bianMa)
				}
				$("#Form").submit();
				$("#zhongxin").hide();
				$("#zhongxin2").show();
...
<body>
	<form  action="dictionaries/save.do" name="Form" id="Form" method="post" >
		...
		<table>
			...
			<tr class="info">
				<td>
					<input type="text" name="BIANMA" id="BIANMA" placeholder="这里输入编码" value="${pd.BIANMA }" title="编码"/></td>
			</tr>
			<tr class="info">
				<td><input type="text" name="DISPLAY_VALUE" id="DISPLAY_VALUE" placeholder="不填写时，默认保存编码" value="${pd.DISPLAY_VALUE }" title="设定值"/></td>
			</tr>
			<tr class="info">
