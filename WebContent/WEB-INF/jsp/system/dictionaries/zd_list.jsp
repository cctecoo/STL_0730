...
<div class="row-fluid">
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<tr>
			...
			<th class='center'>编码(唯一值)</th>
			<th class='center'>设定值</th>
			<th class='center'>级别</th>
			<th class='center'>组合编码</th>
		...
		<c:choose>
			<c:when test="${not empty varList}">
				...
				<td class='center'>${var.DISPLAY_VALUE }</td>
		...
		<table style="width:100%;">
			<tr>
				<td style="vertical-align:top;">
					...
					<c:if test="${pd.PARENT_ID != '0'}">
						<a class="btn btn-small btn-info" onclick="location.href='<%=basePath%>/dictionaries.do?PARENT_ID=${pdp.PARENT_ID }';">返回</a>
					</c:if>
