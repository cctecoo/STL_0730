	...
	<form action="buttons/list.do" method="post" name="Form" id="Form">
			...
			<tr>
				<td>
					<span class="input-icon">
						<input autocomplete="off" id="keyword" type="text" name="keyword" value="${pd.keyword}" placeholder="这里输入关键词" />
						<i id="nav-search-icon" class="icon-search"></i>
					</span>
				</td>
				<td style="vertical-align:top;">
					<select  id="ENABLED" name="enabled" data-placeholder="请选择状态" style="vertical-align:top;width: 79px;" class="form-field-select-2">
						<option value=""></option>
						<option value="1" <c:if test="${pd.ENABLED == '1' }">selected</c:if> >有效</option>
		...
		<table id="table_report" class="table table-striped table-bordered table-hover">
			...
			<c:choose>
				<c:when test="${not empty varList}">
					...
					<td class='hidden-480'><span class='label label-inverse arrowed-in'>无效</span></td>
					</c:if>
					...
					<c:if test="${QX.edit == 1 }">
					<a class='btn btn-mini btn-info' title="编辑" onclick="editButtons('${var.BUTTONS_ID }');"><i class='icon-edit'></i></a>
					</c:if>
					
					<c:if test="${QX.del == 1 }">
					<a class='btn btn-mini btn-danger' title="删除" onclick="delButtons('${var.BUTTONS_ID }','${var.BUTTONS_NAME }');"><i class='icon-trash'></i></a>
					</c:if>
		...
		<div class="page-header position-relative">
			...
			<table style="width:100%;">
				<tr>
					<td style="vertical-align:top;"><a class="btn btn-small btn-success" onclick="addButtons();">新增</a></td>
				</tr>
			</table>
			</c:if>
		</div>
