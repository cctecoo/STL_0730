...
<html lang="en">
	...
	<body>
								...
								<tr>
									<td>
										<span class="input-icon">
											<input autocomplete="off" id="USERNAME" type="text" name="USERNAME" value="${pd.USERNAME }" style="height:28px;" placeholder="这里输入关键词"/>
										</span>
									</td>
									<td><input class="span10 date-picker" name="lastLoginStart" id="lastLoginStart"  value="${pd.lastLoginStart}" type="text" data-date-format="yyyy-mm-dd" style="width:88px;height:28px;" placeholder="开始日期" title="最近登录开始"/></td>
									<td><input class="span10 date-picker" name="lastLoginEnd" id="lastLoginEnd"  value="${pd.lastLoginEnd}" type="text" data-date-format="yyyy-mm-dd" style="width:88px;height:28px;" placeholder="结束日期" title="最近登录结束"/></td>
									<td style="vertical-align:top;"> 
									 	<select name="ROLE_ID" id="role_id" data-placeholder="请选择职位" style="vertical-align:top;width: 120px;height:28px;">
									...
									<td style="vertical-align:top;"> 
										<span class="input-icon">
											<input autocomplete="off" id="dept_id" name="DEPT_NAME" onclick="showDeptTree(this)" type="text" style="height:28px;" value="${pd.deptNames }" placeholder="点击选择组织机构" />
									...
									<c:if test="${userRole.role.ROLE_ID=='1' || userRole.role.PARENT_ID=='1' }">
							...
							<table id="table_report" class="table table-striped table-bordered table-hover">
								...
								<thead>
									<tr>
										...
										<th align="center" data-align="center" data-width="150px" data-sortable="false" data-formatter="btns">操作</th>
									</tr>
								</thead>
		...
		<script type="text/javascript">
				...
				$("#table_report").bootgrid({
					...
					formatters:{
						...
						btns: function(column, row){
							...
							if('${userRole.role.ROLE_ID}'=='1' || '${userRole.role.PARENT_ID}'=='1'){
								result += '<a class="btn btn-mini btn-info" title="编辑" onclick="editUser(\'' + row.USER_ID +'\');"><i class="icon-edit"></i></a>';
								result += '<a class="btn btn-mini btn-danger" title="删除" onclick="delUser(\'' + row.USER_ID + '\',\'' + 
										row.USERNAME + '\', ' + row.createdDays + ');"><i class="icon-trash"></i></a>';
							}
			...
			//删除
			function delUser(userId, msg, createdDays){
				if(null==createdDays || createdDays>5){
					top.Dialog.alert("用户创建超过五天，不能删除");
					return false;
				}
					...
					if(result) {
						...
						for(var i=0;i < document.getElementsByName('ids').length;i++){
							if(document.getElementsByName('ids')[i].checked){
							  	...
							  	if(emstr=='') 
							  		emstr += document.getElementsByName('ids')[i].id;
							  	else 
							  		emstr += ';' + document.getElementsByName('ids')[i].id;
							  	
							  	if(phones=='') 
							  		phones += document.getElementsByName('ids')[i].alt;
							  	else 
							  		phones += ';' + document.getElementsByName('ids')[i].alt;
							}
						}
