...
<script type="text/javascript">
	...
	function openClose(menuId,curObj,trIndex){
		...
		if(txt=="展开"){
			$(curObj).text("折叠");
            $(curObj).parents('tr').after("<tr id='tempTr"+menuId+"'><td colspan='5'>数据载入中</td></tr>");
			if(trIndex%2==0){
				$("#tempTr"+menuId).addClass("main_table_even");
			}
			//判断当前菜单有几个展开图片
			var prevImgCount = $($(curObj).parent().parent().find("td")[1]).find("img").length;
			//根据展开图片的个数，设置下一级菜单的左边距
			var styleHtml = " style='vertical-align: middle; margin-left: " + (10*prevImgCount) + "px' ";
			...
			$.get(url,function(data){
				if(data.length>0){
					...
					$.each(data,function(i){
						...
						html += "<td></td>";
						html += "<td><span style='width:80px;'></span>";
						if(i==data.length-1)
							html += "<img src='static/images/joinbottom.gif' " + styleHtml + " />";
						else
							html += "<img src='static/images/join.gif' " + styleHtml + " />";
						html += "<span style='width:100px;text-align:left;display:inline-block;'>"+this.MENU_NAME+"</span>";
						html += "</td>";
						html += "<td>"+this.MENU_URL+"</td>";
						html += "<td class='center'>"+this.MENU_CODE+"</td>";
						html += "<td class='center'>"+this.MENU_ORDER+"</td><td>";
                        if(this.MENU_URL == '#'){
                            html += "<a class='btn btn-mini btn-warning' onclick='openClose(\""+this.MENU_ID+"\",this," + (i+1).toString() + ")'>展开</a>"
                        }
						html += "<a class='btn btn-mini btn-info' title='编辑' onclick='editmenu(\""+this.MENU_ID+"\")'><i class='icon-edit'></i></a> <a class='btn btn-mini btn-danger' title='删除' onclick='delmenu(\""+this.MENU_ID+"\",false)'><i class='icon-trash'></i></a></td>";
						html += "</tr>";
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<c:choose>
			<c:when test="${not empty menuList}">
				...
				<td class='center'>${menu.MENU_CODE }</td>
