...
<script type="text/javascript">
	...
	function add(NEWS_ID){
		 ...
		 diag.Drag=true;
		 diag.Title ="新增";
		 diag.URL = '<%=basePath%>/dictionaries/toAdd.do?NEWS_ID='+NEWS_ID;
		 diag.Width = 223;
		 diag.Height = 175;
...
<div class="row-fluid">
	...
	<!-- 检索  -->
	<form action="<%=basePath%>news/list.do" method="post" name="newsForm" id="newsForm">
	...
	<table id="table_report" class="table table-striped table-bordered table-hover">
		...
		<c:choose>
			<c:when test="${not empty varList}">
				...
				<td class='center'><a href="<%=basePath%>/news/mlist.do?NEWS_ID=${var.ID }" title="查看下级">
				<i class="icon-arrow-right  icon-on-right"></i>&nbsp;${var.NOTICE }</a></td>
				</tr>
				</c:forEach>
			</c:when>
			<c:otherwise>
		...
		<table style="width:100%;">
			<tr><!--
				<td style="vertical-align:top;" class="left"><a class="btn btn-small btn-info" onclick="location.href='<%=basePath%>/dictionaries.do?NEWS_ID=${pdp.NEWS_ID }';">返回</a></td>
