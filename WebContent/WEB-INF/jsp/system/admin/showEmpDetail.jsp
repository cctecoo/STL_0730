...
ul{ list-style:none;} 
		.infoTbl td{height:40px;}
		.infoTbl td input{height:30px; padding:5px; margin-bottom:5px; border:0;}
		#myTab{margin-bottom:0;}
		
		#myTab > li{ background:#98c0d9; margin:0; border:2px solid #98c0d9;}
		#myTab > li.active{ border:2px solid #448fb9;}
		#myTab > li.active a{ margin:0;box-shadow:none!important; background:#448fb9;}
		#myTab > li.active a:hover{ background:#448fb9; }
		#myTab > li a{color:#fff; padding:12px 25px;}
		#myTab > li a:hover{color:#fff; background:#98c0d9;}
		.dropdown-menu > li{color:#98c0d9!important; background:#fff!important;}
		.dropdown-menu > li a{color:#98c0d9!important; background:#fff!important;}
		.dropdown-menu > li.active a{color:#fff!important; background:#448fb9!important;}
</style> 
<script> 
/*第一种形式 第二种形式 更换显示样式*/ 
...
<li class="dropdown">
	<a href="dropdown-toggle" data-toggle="dropdown">员工任务<i class="icon-caret-down bigger-110 width-auto"></i></a>
...
<li><a href="#con_one_5" data-toggle="tab">员工培训记录</a></li> 
...
<div class="table-responsive">
					...
					<div style="float: left; margin:15px;">
						<!-- 
						员工编号:<input type="text" id="EMP_CODE1" readonly="readonly" style="width:150px; height:30px; margin-left:5px; margin-right:20px; padding-left:10px;">
						 -->
						员工姓名:<input type="text" id="EMP_NAME1" readonly="readonly" style="width:150px; height:30px; margin-left:5px; margin-right:20px; padding-left:10px;">
						所属部门:<input type="text" id="DEPT_NAME1" readonly="readonly" style="width:150px; height:30px; margin-left:5px; margin-right:20px; padding-left:10px;">
					</div>
					...
					<table id="grid1" class="table table-striped table-bordered table-hover">
						...
						<thead>
							<tr>
								...
								<th data-column-id="COUNT" data-formatter="targetCount">目标数量</th>
								<!-- 
								<th data-column-id="UNIT_NAME">单位</th>
								<th data-column-id="DEPT_NAME">承接部门</th>
								<th data-column-id="EMP_NAME">承接责任人</th>
								 -->
								<th data-column-id="STATUS_NAME" data-visible="false">状态</th>
							</tr>
						</thead>
					</table>
...
<div id="con_one_2" class="tab-pane fade">
	<div class="table-responsive">
					...
					<div style="float: left; margin:15px;">
						员工编号:<input type="text" id="EMP_CODE2" readonly="readonly" style="width:150px; height:30px; margin-left:5px; margin-right:20px; padding-left:10px;">
						员工姓名:<input type="text" id="EMP_NAME2" readonly="readonly" style="width:150px; height:30px; margin-left:5px; margin-right:20px; padding-left:10px;">
						所属部门:<input type="text" id="DEPT_NAME2" readonly="readonly" style="width:150px; height:30px; margin-left:5px; margin-right:20px; padding-left:10px;">
					</div>
...
<div id="con_one_21" class="tab-pane fade">
								<div class="table-responsive">
									<table id="task_grid_0" class="table table-striped table-bordered table-hover" >
										<thead>
											<tr>
												<th data-width="50px" data-column-id="ID" data-visible="false" data-formatter="id" data-css-class="aa" data-sortable="true">ID</th>
...
<div id="con_one_22" class="tab-pane fade">
								<div class="table-responsive">
									<table id="task_grid_1" class="table table-striped table-bordered table-hover">
										<thead>
											<tr>
												<th data-width="50px" data-column-id="ID" data-visible="false" data-formatter="id" data-css-class="" data-sortable="true">ID</th>
								...
								<div class="table-responsive">
									<table id="task_grid_2" class="table table-striped table-bordered table-hover">
										<thead>
											<tr>
												<th data-width="50px" data-column-id="ID" data-visible="false" data-formatter="id" data-css-class="" data-sortable="true">ID</th>
												<th data-width="15%" data-column-id="NODE_NAME" data-formatter="nodeName">节点名称</th>
												...
												<c:if test="${empty pd.showDept }">
													<th data-width="200px" class="center" data-formatter="commandStr" data-sortable="false">操作</th>
												</c:if>
											</tr>
										</thead>
									</table>
...
<div id="con_one_5" class="tab-pane fade">
	<div class="table-responsive">
		<table id="grid5" class="table table-striped table-bordered table-hover">
						<thead>
							<tr>
							<th data-column-id="EMP_NAME" data-width="60px" data-sortable="false">员工</th>
							<th data-column-id="START_DATE" data-formatter="startDate" data-width="90px">培训日期</th>
							<th data-column-id="TRAIN_CONTENT" data-formatter="trainContent">培训内容</th>
							<th data-column-id="TRAIN_TEACHER" data-width="60px">讲授人</th>
							...
							<th data-column-id="LESSON_PERIOD" data-width="50px">课时</th>
	...
	<script type="text/javascript">
		$(function(){
			...
			$.get(url,function(data){
		        ...
		        $("#grid1").bootgrid({
					...
					navigation:2,
					formatters:{
						targetCount: function(column, row){
							return '<span title="' + row.COUNT + '(' + row.UNIT_NAME + ')' + '">' + row.COUNT + '(' + row.UNIT_NAME + ')' + '</span>';
						}
					}
				});
			 	//表格数据获取(tab2)
		        ...
		        $("#grid5").bootgrid({
					ajax: true,
					url:"empTrain/empTrainList.do?empCode="+empCode,
					navigation:2,
					formatters:{
			...
			$("#task_grid_"+i).bootgrid({
				...
				formatters:{
					...
					nodeName: function(column, row){
						var str = "";
						if(row.LATE=="true"){
							str = '<span class="glyphicon glyphicon-time" style="color: #f55273; font-size:16px" title="超过1天没有接收"></span>';
						}
						if('已完毕'==row.STATUS_NAME){
							return '<a title="' + row.NODE_NAME + '" onclick="showNodeDetail(' + row.ID + ')">' + row.NODE_NAME + '</a>';
						}else{
							return str+'<span title="' + row.NODE_NAME + '">' + row.NODE_NAME + '</span>';
						}
						
					},//流程名称-流程工作
					flowName: function(column, row){
						//用于显示处理流程时的标题
						var flowNameStr = row.FLOW_NAME.replace(/\n/g,'').replace(/\r/g, '');
						var dialogName = "\' " + flowNameStr + " " + row.OPERA_TIME + "\'";
						return '<a title="' + flowNameStr + '" onclick="shwoFlowDetail(' + dialogName + ', ' + row.FLOW_ID + ')">' + flowNameStr + '</a>';
					}
				}
			});
		}
		...
		//显示流程信息
		function shwoFlowDetail(dialogName, id){
			top.jzts();
			var diag = new top.Dialog();
			diag.Drag=true;
			diag.Title ="流程明细" + dialogName;
		...
		function showNodeDetail(workNodeId){
			...
			diag.Drag=true;
			diag.Title ="节点岗位职责明细";
			diag.URL = '<%=basePath%>flowWork/showNodeDetail.do?workNodeId=' + workNodeId;
			diag.Width = 700;
			diag.Height = 400;
			diag.show();
		}
