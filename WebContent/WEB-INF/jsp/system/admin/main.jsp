...
<!--[if lt IE 9]>
		...
		body:before{background:url(static/navigation/assets/images/hp_bg.png) 100% no-repeat; }
		td,th{font-size:12px; color:#585858;}
		th{background:#f7f8fa;}
		.modal-backdrop{ z-index:99;}
		input[type=checkbox]{opacity:1; position:inherit;}
		#myTab{margin-bottom:0;}
		#myTab>ul>li{ background:#98c0d9; margin:0; border:2px solid #98c0d9;}
		#myTab > li.active{ border:2px solid #448fb9;}
		#myTab>ul>li.active a{ margin:0;box-shadow:none!important; background:#448fb9;}
		#myTab>ul>li.active a:hover{ background:#448fb9; }
		#myTab>ul>li a{color:#fff; padding:12px 25px;}
		#myTab>ul>li a:hover{color:#fff; background:#98c0d9;}
		#myTab>ul>li>ul>li{color:#98c0d9; background:#fff;}
		#myTab>ul>li>ul>li a{color:#98c0d9; background:#fff;}
		.tabtitle{ border:1px solid #ddd; line-height:40px; border-bottom:0;}
...
max-width:none;
		...
		<style>
		.hp_left{width:200px; float:left; margin:30px 0 0 20px;}
		.byregion{ margin-bottom:20px;}
		.byregion ul{ margin:0;} 
		.byregion li{float:left; list-style-type:none;}
		.byregion a{color:#1d1d1d; padding:5px 10px;}
		.byregion a:hover{ text-decoration:none;}
		.byfunction{ margin-bottom:20px;}
		.byfunction ul{ margin:0; position:relative;} 
		.byfunction>ul>li{float:left; list-style-type:none;margin:15px 0;}
		.byfunction>ul>li:hover{background:#0574b7; color:#fff;}
		.byfunction>ul>li>ul{display:none; position:absolute; left:0; padding:10px; width:150px; border:1px solid #ccc;background:#fff;}
		.byfunction>ul>li:hover>ul{display:block}
		
		
		.byfunction a{color:#1d1d1d; padding:5px 10px;}
		
		.byfunction a:hover{ background:#0574b7; color:#fff;}
		/*
		.byfunction a:hover{ text-decoration:none;}
		.byfunction>ul>li>ul>li{ list-style-type:none;}
		#function_list{ display:none; position:absolute; left:0; padding:10px; width:150px; border:1px solid #ccc;background:#fff;}
		#function_list a{color:#616161;}
		*/
		.hp_right{margin-left:220px;}
	...
	<div class="row">
		<div class="col-xs-12" style="margin-left:-1px;">
			<div id="winimg" style="width:100%; overflow:hidden;">
				<img src="static/logo_change/indexbanner.jpg"
					style="width:100%;">
					<!-- 
						...
						<c:choose>
							<c:when test="${not empty functionList }">
								<c:forEach items="${functionList }" var="item" varStatus="vs">
									<li><a href="javascript:"onclick="showDeptByFun('${item.BIANMA }')">${item.NAME}</a>
										<ul id="function_list">
											<c:choose>
												<c:when test="${not empty allDeptList}">
													<c:forEach items="${allDeptList }" var="dept">
														<c:if test="${item.BIANMA == dept.FUNCTION }">
															<li><a href="javascript:"onclick="openEmp('${dept.ID }')">${dept.DEPT_NAME }</a></li>
														</c:if>
													</c:forEach>
												</c:when>
											</c:choose>
									...
									<c:if test="${vs.index < fn:length(functionList)-1 }">
										<li><span>|</span></li>
									</c:if>
								</c:forEach>
							</c:when>
			...
			<div class="hp_right">
				<iframe name="showDataFrame" id="showDataFrame" frameborder="0"
					...
					style="width: 100%;min-height: 590px;"></iframe>
		</div>
	</div>
