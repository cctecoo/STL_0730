			...
			<c:forEach items="${menuList}" var="menu">
				...
				<c:choose>
					<c:when test="${menu.MENU_URL=='#'}">
                                                            ...
                                                            <c:choose>
                                                                <c:when test="${not empty newSub.MENU_URL}">
                                                                    <li id="z${newSub.MENU_ID}">
                                                                        <a style="cursor:pointer;" target="mainFrame"  onclick="siMenu('z${newSub.MENU_ID }','lm${sub.MENU_ID }','${newSub.MENU_NAME }','${newSub.MENU_URL }')">
                                                                            <i class="icon-double-angle-right"></i>${newSub.MENU_NAME }</a>
                                                                    </li>
                                                                </c:when>
                                                                <c:otherwise>
                                                                    <li><a href="javascript:void(0);"><i class="icon-double-angle-right"></i>${newSub.MENU_NAME }</a></li>
                                                                </c:otherwise>
                                                            </c:choose>
                                                        </c:if>
                                                    </c:forEach>
				  	...
				  	<c:otherwise>
				  		<a style="cursor:pointer;" target="mainFrame"  onclick="siMenu('lm${menu.MENU_ID }','lm${menu.MENU_ID }','${menu.MENU_NAME }','${menu.MENU_URL }')">
	                        <i class="${menu.MENU_ICON == null ? 'icon-desktop' : menu.MENU_ICON}"></i>
	                        <span> ${menu.MENU_NAME }</span>
                        </a>
				  	</c:otherwise>
