...
dTree.prototype.node = function(node, nodeId) {
	...
	if(this.config.useCheckBox && nodeId!=0){
		...
		str += '<input type="checkbox" name="id" id="c'+this.parEnum+'" onClick="javascript:'+this.obj+'.caBox(\''+this.formId+'\',\'c'+this.parEnum+'\')" value="' + node.id +'" class="cx"/>';
	}

    // �˴���getRadioSelected��������ʵ���ڵ�������ҳ��
        if(this.config.useRadio && node.id != 0){
            str +='<input type="radio" name="chk" id="r'+  this.obj + nodeId + '" onclick="getRadioSelected('+nodeId+')" value="'+node.id+'@'+node.name+'"/>';
        }
...
// Returns the selected checkbox node ids
dTree.prototype.getSelectedChecks = function(){
    var ids = [];
    var j=0;
    if(this.config.useCheckBox){
        ...
        ids[0] = this.getSelected();
    }
    //alert("ids length = " + ids.length);
    return ids;
}
// Returns the selected radio node id
dTree.prototype.getRadios = function(){
    var id;
    if(this.config.useRadio){
        var form = document.getElementById(this.formId);
        for (var i=0; i<form.elements.length; i++) {
            var element = form.elements[i];
            if (element.name == "chk" && element.type=='radio'){
                if( element.checked == true ){
                    id = element.value;
                    break;
                }
...
dTree.prototype.getCheckboxId = function(cbid){
    var form = document.getElementById(this.formId);
    var checkboxid;
    for (var i=0; i<form.elements.length; i++) {
        var element = form.elements[i];
...
// ������ڵ�Id����Ϊѡ��
dTree.prototype.setSelectedChecks = function(ids){
    var id = this.getCheckboxId(ids);
    var cb = document.getElementById(id);
    if(cb != null)
        cb.checked = true;
}

dTree.prototype.getRadioId = function(cbid){
    var form = document.getElementById(this.formId);
    var radioid;
    for (var i=0; i<form.elements.length; i++) {
        var element = form.elements[i];
        //alert(element.name+"----"+element.type+"-----"+element.id);
        if (element.name == "chk" && element.type=='radio'){
            //
            radioid = element.id;
            if(cbid == element.value.split("@")[0]){
                break;
            }
        }
    }
...
//
dTree.prototype.setSelectedRadio = function(ids){
    var id = this.getRadioId(ids);
    var cb = document.getElementById(id);
...
/* checkbox tree */
dTree.prototype.caBox = function(formId, regx ) {
    if (this.config.singleSelect) return;
...
function isCheckedByRec( form ,regx){
 ...
 function getRadioSelected(nodeId){
        //var nname = this.aNodes[nodeId].name;   //�˴��õ�������ѡ��ڵ��nameֵ�������Ҫ��ͬ��ɵõ������ֵ
        //document.getElementById('exID').value = "��ѡ���ˣ�" + nname;  
    }
