...
$.fn.initDeptTree = function(setting, depts, height, width){
	var deptTree;
	var deptTreeInner = $(this);;//页面点击显示树的输入框
	
	//objId和treeId均为树元件ID，为兼容其他人用法，保留
	var objId = $(deptTreeInner).attr("id");
	var treeId = setting.treeId ? setting.treeId : "deptTree";
	initForInput(setting, depts, height, width);
	
	function initForInput(setting, depts, height, width){
		var appendHtml = '';
		...
		appendHtml += '<input type="hidden" id="EMP_DEPARTMENT_ID" name="EMP_DEPARTMENT_ID" value="' + initDeptId + '"/>' + 
			...
			'<ul id="' + treeId + '" class="tree"></ul>'
			+'</div>';
		
		$(deptTreeInner).after(appendHtml);
		...
		$(deptTreeInner).bind('click', function(){
