...
jQuery(document).ready(function ($) {
    ...
    $(".mumu").mouseover(function () {
       $(".mumu").fadeTo("300", 0.3);
       msgs = ["日事日毕，日清日高", "我的薪酬我做主！", "人人都经营，事事都创新！", "把手拿开我才出来！"];
...
jQuery(document).ready(function ($) {
    if (isindex) { //如果是主页
        ...
        if (now > 0 && now <= 6) {
            showMessage(visitor + ' 今天完成的事情必须比昨天有质的提高，明天的目标必须比今天更高哦~', 3000);
        } else if (now > 6 && now <= 11) {
            showMessage(visitor + '上午好，今天都要做哪些工作呢?要规划好哦~', 3000);
        } else if (now > 11 && now <= 14) {
            showMessage(visitor + '人人都经营，事事都创新', 3000);
        } else if (now > 14 && now <= 18) {
            showMessage(visitor + ' 中午的时光真难熬！还好有你在！', 3000);
        } else {
            showMessage(visitor + ' 习惯改变一点点', 3000);
        }
    }
    else {
        showMessage('欢迎' + visitor + '来到B5教程网《' + title + '》', 3000);
    }
    $(".spig").animate({
        top: $(window).scrollTop() + 450,
        left:70
        //right: document.body.offsetWidth - 100
    },
	{
	    queue: false
	});
...
jQuery(document).ready(function ($) {
    ...
    window.setInterval(function () {
        msgs = ["今天完成的事情必须比昨天有质的提高，明天的目标必须比今天更高哦~"];
        var i = Math.floor(Math.random() * msgs.length);
